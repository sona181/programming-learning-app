...
export default function Sidebar(): JSX.Element {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const res = await fetch(`/api/user/sidebar?userId=${userId}`);
        if (!res.ok) return;
        ...
        setRegisteredCount((data.courses?.registered ?? []).length);
      } catch (e) {
        console.error(e);
      }
    }
  ...
  const navItems = [
    ...
    },
    {
      ...
      href: "/my-courses",
      icon: (
        // ✅ Open book with bookmark — matches screenshot
        <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
        ...
        >
          ...
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              ...
              <path
                d="M6 11.5v5.5c0 0 2 2.5 6 2.5s6-2.5 6-2.5V11.5"
                stroke="white"
                strokeWidth="1.4"
                fill="none"
        ...
        >
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={userId ? `${item.href}?userId=${userId}` : item.href}
                className="nav-link"
                style={{
                  display: "flex",
                  alignItems: "center",
                  ...
                  // ✅ Active = dark navy highlight, not bright blue
              ...
              >
                ...
                <span
                  className="nav-label"
                  style={{ flex: 1, color: isActive ? "#fff" : "#64748B" }}
                >
                  {item.label}
                </span>

                {item.href === "/my-courses" && registeredCount > 0 && (
                  <span
                    style={{
                      background: "#2563EB",
                      color: "#fff",
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ lineHeight: 1.4, minWidth: 0 }}>
                ...
                <p style={{ color: "#475569", fontSize: 11, margin: 0 }}>
                  {roleLabel} · {user.xp.toLocaleString()} XP
                </p>
              </div>
            </div>
          )}
